...
/// Return true if attribute macro args declares http flavor in the form `#[icp(http)]`
...
pub fn icp(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let result = match inputs.len() {
        ...
        _ => {
            let tokens = quote_spanned! { inputs.span() =>
                compile_error!("The #[icp] macro can expects two arguments: a triggered event and context.");
            };
            return TokenStream::from(tokens);
        }
    };
